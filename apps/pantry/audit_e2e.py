...
if not MONGODB_URI or not JWT_SECRET_VALUE:
...
if not OWNER_ID:
    print("❌ Error: OWNER_ID environment variable must be set.")
    sys.exit(1)
...
async def check_1_db_truth(db):
    ...
    for d in docs:
        print(
            f"    _id={d['_id']}  approval_status={d.get('approval_status')}  visitor_type={d.get('visitor_type')}  is_active={d.get('is_active')}"
        )

    pending = [
        d
        for d in docs
        if d.get("approval_status") == "pending" and d.get("visitor_type") == "regular"
    ]
    ...
    if not docs:
        log(
            1,
            FAIL,
            "No visitor docs found for this owner — write is broken or wrong owner ID",
        )
    elif not pending:
        log(
            1,
            WARN,
            f"Owner has {len(docs)} visitors but 0 pending regulars",
...
async def check_2_api_response(owner_token):
    ...
    try:
        resp = requests.get(
            url, headers={"Authorization": f"Bearer {owner_token}"}, timeout=10
        )
        print(f"  Status: {resp.status_code}")
        data = resp.json()
        print(
            f"  Response body (first 3): {json.dumps(data[:3], default=str, indent=2)}"
        )
...
async def check_3_query_debug(db):
    ...
    if len(b) > 0:
        ...
        log(
            3,
            FAIL,
            "Docs exist without visitor_type filter — visitor_type field is missing or wrong value in DB",
        ...
        log(
            3,
            WARN,
            "No results in any variant — no pending visitors exist yet OR owner ID mismatch",
        )
    else:
        log(3, FAIL, "Query mismatch — see detailed output above")


async def check_4_sse_connection(_db):
    ...
    try:
        ...
        if resp.status_code == 200:
            log(
                4,
                PASS,
                "SSE /events/stream endpoint is reachable and returns 200",
        ...
        log(
            4,
            PASS,
            "SSE endpoint is live (connection held open for streaming — expected timeout)",
        )
    ...
    try:
        ...
        try:
            ...
            log(
                5,
                SKIP,
                f"Backend is NOT reachable at {API_BASE} — start Pantry and re-run",
            )
    ...
    try:
        ...
        if resp.status_code != 200:
            log(
                6,
                SKIP,
                f"Visitor list returned {resp.status_code} — guard token may be needed",
            )
            return
        try:
            data = resp.json()
        ...
        pending = [
            ...
            and v.get("visitor_type") == "regular"
        ]
        ...
        if len(pending) > 0:
            log(
                6,
                PASS,
                f"Pending regular visitors visible in /visitors/ list: {len(pending)}",
            )
        else:
            log(
                6,
                WARN,
                "No pending regulars in /visitors/ list (owner filter may restrict this)",
            )
    except (requests.ConnectionError, requests.ReadTimeout, requests.Timeout):
    ...
    async for doc in db.visitors.aggregate(pipeline):
        ...
        if status not in valid:
            ...
            log(
                7,
                FAIL,
                f"Invalid status '{status}' in {count} DB documents",
...
async def check_8_field_presence(db):
    ...
    print(
        f"  flat_id stored: {'YES — ' + doc.get('flat_id', '') if 'flat_id' in doc else 'NO (not stored)'}"
    )
...
async def main():
    ...
    owner_token = make_owner_token()
    print("[OK] Owner token generated (expires in 1h)")
