...
function PhotoUnavailable({
  ...
  return (
    <>
      ...
      <button
        type="button"
        title={`${label} - tap to take it again`}
        aria-label={`${label} - tap to take it again`}
      ...
      >
        {body}
        {!isUploading && (
          <span className="absolute -bottom-0.5 -right-0.5 flex h-1/3 w-1/3 min-h-[14px] min-w-[14px] items-center justify-center rounded-full bg-primary text-primary-foreground">
...
export default function SecureImage({
  ...
  // Cloudinary URLs benefit from a resize on the way out. Signed API URLs,
  // blobs and data URIs stay on a plain <img>.
